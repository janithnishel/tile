...
class _PurchaseOrderScreenState extends State<PurchaseOrderScreen>
  ...
  void _showSupplierManagementDialog() {
    showDialog(
      context: context,
      builder: (context) => const SupplierManagementDialog(),
    );
  }
  ...
  Future<void> _addSupplier(Supplier supplier) async {
    try {
      // Use the SupplierCubit to create supplier
      final supplierCubit = context.read<SupplierCubit>();
      final createdSupplier = await supplierCubit.createSupplier(supplier);

      // Add to mock data for immediate UI update (in production, you might refresh from API)
